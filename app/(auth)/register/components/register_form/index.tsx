...
const PHONE_REGEX = /^(03[2-9]|05[689]|07[06789]|08[1-5]|09[0-9])\d{7}$/
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fullNameTrimmed = fullName.trim().replace(/\s+/g, ' ')
    ...
    // 2. Số điện thoại
    if (!phoneTrimmed) {
      showToast('warning', 'Thiếu thông tin', 'Vui lòng nhập số điện thoại')
      return
    }
    ...
    if (!PHONE_REGEX.test(phoneTrimmed)) {
      showToast(
        'warning',
        'Số điện thoại chưa hợp lệ',
        'Số điện thoại phải có đầu số hợp lệ tại Việt Nam',
      )
      return
    }
    ...
    // 3. Email
    if (!emailTrimmed) {
      showToast('warning', 'Thiếu thông tin', 'Vui lòng nhập email')
      return
    }
    ...
    if (password.length < 6) {
      showToast(
        ...
        'Mật khẩu phải có ít nhất 6 ký tự, phải bao gồm chữ in hoa, chữ thường, số và ký tự đặc biệt',
      )
      return
    }
    ...
    // 5. Xác nhận mật khẩu
    if (!confirmPassword) {
      showToast('warning', 'Thiếu thông tin', 'Vui lòng nhập xác nhận mật khẩu')
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast('error', 'Đăng ký thất bại', msg, 5000)
        return
      }
      ...
      showToast(
        'success',
        'Đăng ký thành công',
        'Tài khoản đã được tạo. Đang chuyển hướng đến trang đăng nhập...',
        2200,
        true,
      )
          ...
          >
            ...
            >
              <div className="welcome-content">
                ...
                >
                  Tham gia cùng chúng tôi để trải nghiệm những dịch vụ tốt nhất
                  và đặt bàn một cách dễ dàng.
                </motion.p>
            ...
            >
              <div className="glass-card register-card">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <motion.button
                    className="glass-button register-button"
                    disabled={loading}
                    type="submit"
                ...
                <div className="glass-footer">
                  <p>
                    Đã có tài khoản?{' '}
                    <Link className="glass-link" href="/login">
                      Đăng nhập ngay
                    </Link>
                  </p>
                </div>
