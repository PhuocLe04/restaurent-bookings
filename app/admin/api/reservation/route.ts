  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      if (services.length) {
        ...
        const countS = await tx.services.count({ where: { id: { in: sIds } } })
      ...
      if (services.length) {
        // nếu không truyền unit_price -> lấy price hiện tại của service
    ...
    return NextResponse.json(
      { message: 'Create reservation failed', error: String(e?.message ?? e) },
      { status: 400 },
    )
  }
}
